...
void vmin_vv_u32_golden(uint32_t *a,uint32_t *b,uint32_t *exp_result) {
     for (int i = 0; i < ELE_NUM; i++) {
        exp_result[i] = a[i]<b[i] ? a[i]:b[i];
  }
...
int main(void) {
    ...
    //Get golden result
    vmin_vv_u32_golden(exp_a,exp_b,exp_result);

    //Get Intrinsic result
    result = vmin_vv_u32(a,b);
