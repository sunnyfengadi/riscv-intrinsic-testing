...
void vmin_vx_u16_golden(uint16_t *a,uint16_t b,uint16_t *exp_result) {
     for (int i = 0; i < ELE_NUM; i++) {
        exp_result[i] = a[i]<b ? a[i]:b;
  }
...
int main(void) {
    ...
    //Get golden result
    vmin_vx_u16_golden(exp_a,exp_b,exp_result);

    //Get Intrinsic result
    result = vmin_vx_u16(a,b);
