...
void vwmul_vv_u16_m_golden(uint64_t *mask,uint32_t maskoff[2][16],uint16_t *a,uint16_t *b,uint32_t exp_result[][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vwmul_vv_u16_m_golden(exp_mask,exp_maskoff,exp_a,exp_b,exp_result);

    //Get Intrinsic result
    result = vwmul_vv_u16_m(mask,maskoff,a,b);
