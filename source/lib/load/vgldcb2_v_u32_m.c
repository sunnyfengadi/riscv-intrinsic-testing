...
void vgldcb2_v_u32_m_golden(uint64_t *mask,uint32_t maskoff[2][16],uint32_t *base,int32_t *index,uint32_t exp_result[][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vgldcb2_v_u32_m_golden(exp_mask,exp_maskoff,exp_base,exp_index,exp_result);

    //Get Intrinsic result
    result = vgldcb2_v_u32_m(mask,maskoff,base,index);
