...
void vgldcb2_v_u64_m_golden(uint64_t *mask,uint64_t maskoff[2][8],uint64_t *base,int64_t *index,uint64_t exp_result[][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vgldcb2_v_u64_m_golden(exp_mask,exp_maskoff,exp_base,exp_index,exp_result);

    //Get Intrinsic result
    result = vgldcb2_v_u64_m(mask,maskoff,base,index);
