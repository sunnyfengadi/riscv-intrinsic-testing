...
void vldstcb1_v_i64_m_golden(uint64_t *mask, int64_t *base,int64_t*index,int64_t *exp_result) {
...
int main(void) {
    ...
    int64x8_t result = {0};
    int64x8_t tmp_result = {0};
    int64x8_t tmp2_result = {0};
    int64_t exp_result[ELE_NUM] = {0};
    ...
    //Get golden result
    vldstcb1_v_i64_m_golden(exp_mask,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb1_v_i64_m(mask,exp_base,index);
    ...
    vstcb1in0_v_i64_m(mask, base, tmp_result, NORMAL);
    tmp2_result = vldcb1in0_v_i64_m(mask, base, NORMAL);
    vsstcb1_v_i64_m(mask, base2, index, tmp2_result);
    result = vgldcb1_v_i64_m(mask, base2,index);
