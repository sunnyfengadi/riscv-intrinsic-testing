...
void vldstcb1_v_u16_golden(uint16_t *base,uint16_t*index,uint16_t *exp_result) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb1_v_u16_golden(exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb1_v_u16(exp_base,index);
    ...
    vstcb1in0_v_u16(base, tmp_result, NORMAL);
    tmp2_result = vldcb1in0_v_u16(base, NORMAL);
    vsstcb1_v_u16(base2, index, tmp2_result);
    result = vgldcb1_v_u16(base2,index);
