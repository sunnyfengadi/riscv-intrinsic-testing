...
void vldstcb1_v_u16_m_golden(uint64_t *mask, uint16_t *base,uint16_t*index,uint16_t *exp_result) {
...
int main(void) {
    ...
    uint16x32_t tmp_result = {0};
    uint16x32_t tmp2_result = {0};
    ...
    //Get golden result
    vldstcb1_v_u16_m_golden(exp_mask,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb1_v_u16_m(mask,exp_base,index);
    ...
    vstcb1in0_v_u16_m(mask, base, tmp_result, NORMAL);
    tmp2_result = vldcb1in0_v_u16_m(mask, base, NORMAL);
    vsstcb1_v_u16_m(mask, base2, index, tmp2_result);
    result = vgldcb1_v_u16_m(mask, base2,index);
