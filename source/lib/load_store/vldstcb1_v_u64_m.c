...
void vldstcb1_v_u64_m_golden(uint64_t *mask, uint64_t *base,uint64_t*index,uint64_t *exp_result) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb1_v_u64_m_golden(exp_mask,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb1_v_u64_m(mask,exp_base,index);
    ...
    vstcb1in0_v_u64_m(mask, base, tmp_result, NORMAL);
    tmp2_result = vldcb1in0_v_u64_m(mask, base, NORMAL);
    vsstcb1_v_u64_m(mask, base2, index, tmp2_result);
    result = vgldcb1_v_u64_m(mask, base2,index);
