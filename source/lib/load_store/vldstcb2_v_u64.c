...
void vldstcb2_v_u64_golden(unsigned int combo_stride,uint64_t *base, uint64_t *index, uint64_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb2_v_u64_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb2_v_u64(exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb2in0_v_u64(base, tmp_result, NORMAL);
    tmp2_result = vldcb2in0_v_u64(base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb2_v_u64(base2, index, tmp2_result);
    result = vgldcb2_v_u64(base2,index);
