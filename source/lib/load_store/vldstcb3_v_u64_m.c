...
void vldstcb3_v_u64_m_golden(unsigned int combo_stride,uint64_t *mask,uint64_t *base, uint64_t *index, uint64_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    uint64x8x3_t result = {0};
    uint64x8x3_t tmp_result = {0};
    uint64x8x3_t tmp2_result = {0};
    ...
    //Get golden result
    vldstcb3_v_u64_m_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_mask,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb3_v_u64_m(mask,exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb3in0_v_u64_m(mask, base, tmp_result, NORMAL);
    tmp2_result = vldcb3in0_v_u64_m(mask, base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb3_v_u64_m(mask, base2, index, tmp2_result);
    result = vgldcb3_v_u64_m(mask, base2,index);
