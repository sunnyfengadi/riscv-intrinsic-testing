...
void vldstcb4_v_u16_golden(unsigned int combo_stride,uint16_t *base, uint16_t *index, uint16_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb4_v_u16_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb4_v_u16(exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb4in0_v_u16(base, tmp_result, NORMAL);
    tmp2_result = vldcb4in0_v_u16(base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb4_v_u16(base2, index, tmp2_result);
    result = vgldcb4_v_u16(base2,index);
