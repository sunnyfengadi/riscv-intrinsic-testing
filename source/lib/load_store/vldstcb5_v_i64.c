...
void vldstcb5_v_i64_golden(unsigned int combo_stride,int64_t *base, int64_t *index, int64_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb5_v_i64_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb5_v_i64(exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb5in0_v_i64(base, tmp_result, NORMAL);
    tmp2_result = vldcb5in0_v_i64(base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb5_v_i64(base2, index, tmp2_result);
    result = vgldcb5_v_i64(base2,index);
