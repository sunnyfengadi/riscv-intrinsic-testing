...
void vldstcb6_v_i16_golden(unsigned int combo_stride,int16_t *base, int16_t *index, int16_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb6_v_i16_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb6_v_i16(exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb6in0_v_i16(base, tmp_result, NORMAL);
    tmp2_result = vldcb6in0_v_i16(base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb6_v_i16(base2, index, tmp2_result);
    result = vgldcb6_v_i16(base2,index);
