...
void vldstcb7_v_u32_golden(unsigned int combo_stride,uint32_t *base, uint32_t *index, uint32_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    //Get golden result
    vldstcb7_v_u32_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb7_v_u32(exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb7in0_v_u32(base, tmp_result, NORMAL);
    tmp2_result = vldcb7in0_v_u32(base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb7_v_u32(base2, index, tmp2_result);
    result = vgldcb7_v_u32(base2,index);
