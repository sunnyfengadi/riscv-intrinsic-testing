...
void vldstcb8_v_i32_m_golden(unsigned int combo_stride,uint64_t *mask,int32_t *base, int32_t *index, int32_t exp_result[COMBO_NUM][ELE_NUM]) {
...
int main(void) {
    ...
    int32x16x8_t tmp_result = {0};
    int32x16x8_t tmp2_result = {0};
    ...
    //Get golden result
    vldstcb8_v_i32_m_golden((combo_stride * BYTE_NUM + VLEN) / BYTE_NUM,exp_mask,exp_base, exp_index, exp_result);

    //Get Intrinsic result
    tmp_result = vgldcb8_v_i32_m(mask,exp_base,index);
    ...
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vstcb8in0_v_i32_m(mask, base, tmp_result, NORMAL);
    tmp2_result = vldcb8in0_v_i32_m(mask, base, NORMAL);
    data_init_scalar (combo_stride, combo_stride, PAGE_NUM / BYTE_NUM / 2);
    vwr_csr(RUGRATS_VMCOMBOSTRIDE, combo_stride * BYTE_NUM + VLEN);
    vsstcb8_v_i32_m(mask, base2, index, tmp2_result);
    result = vgldcb8_v_i32_m(mask, base2,index);
