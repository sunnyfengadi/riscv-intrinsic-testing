...
void vsnmsacq_vv_i32_golden(int32_t *a,int32_t *b,int32_t *c,int32_t *exp_result) {
     for (int i = 0; i < ELE_NUM; i++) {
        exp_result[i] = -(b[i]*c[i])+a[i];
  }
...
int main(void) {
    ...
    //Get golden result
    vsnmsacq_vv_i32_golden(exp_a,exp_b,exp_c,exp_result);

    //Get Intrinsic result
    result = vsnmsacq_vv_i32(a,b,c);
