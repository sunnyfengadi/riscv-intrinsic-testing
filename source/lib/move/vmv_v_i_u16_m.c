...
void vmv_v_i_u16_m_golden(uint64_t *mask,uint16_t *maskoff,int16_t imm,uint16_t *exp_result) {
...
int main(void) {
    ...
    imm = exp_imm = 0; // imm and exp_imm do not need to call data_init, 0xffff);

    //Get golden result
    vmv_v_i_u16_m_golden(exp_mask,exp_maskoff,exp_imm,exp_result);

    //Get Intrinsic result
    result = vmv_v_i_u16_m(mask,maskoff,0);
