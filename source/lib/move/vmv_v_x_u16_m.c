...
void vmv_v_x_u16_m_golden(uint64_t *mask,uint16_t *maskoff,uint16_t a,uint16_t *exp_result) {
...
int main(void) {
    ...
    data_init_scalar(a, exp_a, 0xffff);

    //Get golden result
    vmv_v_x_u16_m_golden(exp_mask,exp_maskoff,exp_a,exp_result);

    //Get Intrinsic result
    result = vmv_v_x_u16_m(mask,maskoff,a);
