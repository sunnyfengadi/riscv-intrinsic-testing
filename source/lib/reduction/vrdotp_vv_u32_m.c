...
void vrdotp_vv_u32_m_golden(uint64_t *mask[2][16],uint32_t *a,uint32_t *b,uint32_t *c,uint32_t *exp_result) {
Operator Line --- TODO
}
...
int main(void) {
    ...
    data_init_bool(mask, exp_mask, 16,2, 0xffffffff);
    data_init(a, exp_a, 16, 0xffffffff);
    data_init(b, exp_b, 16, 0xffffffff);
    data_init(c, exp_c, 16, 0xffffffff);

    //Get golden result
    vrdotp_vv_u32_m_golden(exp_mask,exp_a,exp_b,exp_c,exp_result);

    //Get Intrinsic result
    result = vrdotp_vv_u32_m(mask,a,b,c);
