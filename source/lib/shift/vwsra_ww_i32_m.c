...
void vwsra_ww_i32_m_golden(uint64_t *mask,int16_t *maskoff,int32_t *a,uint32_t *b,int32_t *exp_result) {
     for (int i = 0; i < ELE_NUM; i++) {
        exp_result[i] = mask[i]>>maskoff[i];
  }
...
int main(void) {
    ...
    data_init_bool(mask, exp_mask, 32, 0xffffffff);
    data_init(maskoff, exp_maskoff, 32, 0xffff);
    data_init(a, exp_a, 16, 0xffffffff);
    data_init(b, exp_b, 16, 0xffffffff);

    //Get golden result
    vwsra_ww_i32_m_golden(exp_mask,exp_maskoff,exp_a,exp_b,exp_result);

    //Get Intrinsic result
    result = vwsra_ww_i32_m(mask,maskoff,a,b);
